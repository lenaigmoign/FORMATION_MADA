...
> Exercice en groupes : rendre lisible le code ci-dessous
...
# Étape 5 : Résumez les données
ventes_par_produit <- ventes_magasin %>%
  group_by(produit) %>%
  summarise(
    quantite_totale = sum(quantite),
    montant_total = sum(montant),
    nombre_ventes = n() #permet de compter le nombre d'observations dans un groupe de données
  )
...
# 4. Import de données attributaires
...
ggplot(data = AP_superficie_annees, 
       aes(x = an_creation, y = superficie_cumulée)) +
  geom_point() +
  geom_line() +
...
> Exercice : comment auriez-vous comparé vos jeux de données Vahatra et
> WDPA_Mada ?
...
```{r}
AP_Vahatra <- st_read("data/Vahatra/AP_Vahatra.geojson", quiet = TRUE) %>%
    rename(cat_iucn = cat_iucn) # une variable a un nom étrange : on simplifie
...
> Observer et analyer : quelles sont les différences entre ces deux jeux
> de données.
